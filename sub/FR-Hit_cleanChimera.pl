...
while (my $l = <IN>){
  ...
  $array[7] =~ s/\%$//;
  $array[1] =~ s/nt$//;
  # Next if the percent id is less than 95%
  next if ($array[7] < 95);
  # Next unless the percent covered is over 95% of the read or is in the first or last 30bp
  next unless ((($array[3]/$array[1]) >= 0.95) or ($array[-2]< 5) or ($array[-1] > ($length{$array[8]}-5)));
  for (my $i=$array[-2]; $i<=$array[-1]; $i++){
    # Marks the coverage of all sequences
    $genomes{$array[8]}[$i-1][0]++;
